...
run_one_dataset_sc_N <- function(ds_id, save_dir,
                                       sim_long     = sim_obs_sc_N,
    ...
    return(tryCatch({
      ...
      t_pkg <- system.time(
        test_i <- package_scm_result(
          label = sprintf("scn%02d_%s_N%d", scenario_id, ds_tag, sample_N),
          scm_res     = res_i,
...
ds_id       <- as.integer(args[1])
scenario_id <- if (length(args) >= 2) as.integer(args[2]) else 16L
sample_N    <- if (length(args) >= 3) as.integer(args[3]) else 80L

sim_path <- sprintf("Inputdataset/sim_obs_N%d/sim_obs_scenario_%02d.rds",
                    sample_N, scenario_id)
save_dir <- sprintf("output/output_N%d/output_N%d_sc%02d",
                    sample_N, sample_N, scenario_id)

sim_obs_sc_N <- readRDS(sim_path)


message(sprintf("Running dataset ds_id = %d", ds_id))

t_run <- system.time(
  result <- run_one_dataset_sc_N(
    ds_id       = ds_id,
    save_dir    = save_dir,
    sim_long    = sim_obs_sc_N,
    scenario_id = scenario_id,
